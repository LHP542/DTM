...
/// <c>http://127.0.0.1:&lt;port&gt;</c>; alle UI-Zugriffe laufen über
/// <see cref="DtmUiActions"/> auf dem UI-Thread.
///
/// <para>Startet nur, wenn <see cref="ApiOptions.Enabled"/> gesetzt ist.
/// Schlägt der Start fehl (Port belegt o. Ä.), läuft DTM ohne API weiter —
/// eine nicht startende Nebenfunktion darf das Werkzeug nicht blockieren.</para>
///
/// <para><b>Bindet ausschließlich an Loopback.</b> Wer von einem anderen
...
public sealed class ApiHost : IAsyncDisposable
{
    ...
    // Microsoft.Extensions.Logging für die Kestrel-Konfiguration im Scope ist.
    ...
    public async Task StartAsync(ApiOptions options)
    {
        ...
        {
            _logger.Warn("ApiHost läuft bereits.");
            return;
        }
        ...
        try
        {
            ...
            // Kestrel-eigene Logs raus — der Lifecycle wird über NLog
            // protokolliert, alles andere wäre doppelt.
            builder.Logging.ClearProviders();
            builder.Logging.SetMinimumLevel(Microsoft.Extensions.Logging.LogLevel.Warning);

            // CreateSlimBuilder liefert keinen Zugriff auf ConfigureKestrel;
            // der Weg über das Options-Pattern wirkt beim Server-Start.
            ...
            _logger.Info(
                "REST-API läuft auf http://127.0.0.1:{0} (Token {1}, destruktive Aktionen {2})",
                options.Port,
        ...
        {
            _logger.Error(ex, "Start der REST-API fehlgeschlagen — DTM läuft ohne API weiter.");
            _webApp = null;
        }
    }
